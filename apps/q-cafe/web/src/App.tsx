...
export function App() {
  ...
  const [activationStatus, setActivationStatus] = useState<LicenseStatus | null>(null);
  ...
  useEffect(() => {
    const openActivation = () => {
      if (!('__TAURI_INTERNALS__' in window)) {
        setError('License activation is available in the Q Cafe Windows application.');
        return;
      }
      void import('@tauri-apps/api/core')
        ...
        .catch((error) => setError(error instanceof Error ? error.message : 'Q Cafe could not reconnect to the license service.'));
    };
    window.addEventListener('q-cafe-open-license-activation', openActivation);
    return () => window.removeEventListener('q-cafe-open-license-activation', openActivation);
  }, []);
  ...
  function navigate(next: string) {
    if (data?.user?.role === 'cashier' && ['Masters', 'Settings'].includes(next)) {
      location.hash = 'POS';
      setPage('POS');
      return;
    }
  ...
  function signOutExpiredSession(message: string) {
  ...
  async function refresh() {
    try {
      setData(await request<Snapshot>(token));
      setError('');
    } catch (error) {
      const message = String(error);
      if (message.includes('Q Cafe sign-in required.')) {
        signOutExpiredSession('Your Q Cafe session has ended. Please sign in again.');
        return;
      }
      setError(message);
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      void import('@tauri-apps/api/core')
        .then(({ invoke }) => invoke<{ licensed: boolean; activationRequired: boolean }>('qcafe_license_status'))
        .then(status => {
          if (status.licensed || !status.activationRequired) return;
          sessionStorage.removeItem('q-cafe-session');
  ...
  useEffect(() => {
    if (data?.user?.role === 'cashier' && ['Masters', 'Settings'].includes(page)) navigate('POS');
  }, [data?.user?.role, page]);
  async function mutate(path: string, body: unknown) {
    ...
    try {
      if ('__TAURI_INTERNALS__' in window) {
        ...
        const license = await invoke<{ licensed: boolean; activationRequired: boolean; message: string }>('qcafe_license_status');
        if (!license.licensed && license.activationRequired) {
          sessionStorage.removeItem('q-cafe-session');
          setToken('');
          setData(undefined);
      ...
      const response = await request<ActionResult<unknown>>(token, path, body);
      await refresh();
      return response.result;
    } catch (error) {
      const message = String(error);
      if (message.includes('Q Cafe sign-in required.')) signOutExpiredSession('Your Q Cafe session has ended. Please sign in again.');
  ...
  if (showLicenseActivation && activationStatus) {
    return <LicenseActivation status={activationStatus} onActivated={() => window.location.reload()} onClose={() => setShowLicenseActivation(false)} />;
  }
  if (!data) {
    return (
      <Login
