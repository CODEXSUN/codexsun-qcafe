...
export function Masters({ data, mutate }: Props) {
  ...
  const availableSpecials = readTodaySpecialDefinitions(data.master_settings).filter((special) => special.isEnabled);
  ...
  return <div className="h-full min-h-0 w-full flex-1 overflow-y-auto scrollbar-slim p-4 sm:p-6"><div className="flex min-h-full w-full flex-col gap-4">
    ...
    {tab === 'items' ? <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[minmax(18rem,0.8fr)_minmax(32rem,1.2fr)]">
      ...
      <form className="rounded-xl border border-border bg-card" onSubmit={saveItem}><div className="flex items-center justify-between border-b p-4"><div><h2 className="flex items-center gap-2 font-semibold">{selectedId && item.id ? 'Edit item' : 'New item'}{itemDirty && <span className="size-2 rounded-full bg-yellow-400" title="Unsaved changes" aria-label="Unsaved changes"/>}</h2><p className="text-xs text-muted-foreground">Select an item to view it. Use its edit icon to load it into this form.</p></div><Button type="submit" disabled={!itemDirty}><Save size={15}/> Save item</Button></div><div className="grid gap-4 p-4 md:grid-cols-2">
        ...
        <div className="md:col-span-2 rounded-lg border p-3"><div className="mb-3 flex items-center justify-between"><div><b className="text-sm">Special prices</b><p className="text-xs text-muted-foreground">Choose an enabled Today Special, then set this item’s price.</p></div><Button type="button" variant="outline" size="sm" disabled={!todaySpecialEnabled || availableSpecials.length === 0} onClick={() => setItem({...item, specials:[...item.specials,{prefix:'',name:'',price:'',is_enabled:true}]})}><Plus size={14}/> Add special</Button></div>{!todaySpecialEnabled && <p className="mb-3 text-xs text-muted-foreground">Enable Today Special in Settings before adding special item prices.</p>}{todaySpecialEnabled && availableSpecials.length === 0 && <p className="mb-3 text-xs text-muted-foreground">Add and enable a special definition in Settings first.</p>}{item.specials.map((special,index) => <div key={index} className="mb-2 grid gap-2 md:grid-cols-[minmax(10rem,1fr)_8rem_auto]"><select required value={special.prefix} onChange={e => { const selected = availableSpecials.find((definition) => definition.prefix === e.target.value); setItem({...item,specials:item.specials.map((s,i)=>i===index?{...s,prefix:selected?.prefix ?? '',name:selected?.name ?? ''}:s)}); }} className="h-9 rounded-md border bg-background px-2"><option value="">Select Today Special</option>{availableSpecials.map((definition) => <option key={definition.id} value={definition.prefix}>{definition.prefix} · {definition.name}</option>)}</select><input required type="number" min="0.01" step="0.01" placeholder="Price" value={special.price} onChange={e => setItem({...item,specials:item.specials.map((s,i)=>i===index?{...s,price:e.target.value}:s)})} className="h-9 rounded-md border px-2"/><button type="button" aria-label="Remove special price" className="cursor-pointer text-destructive" onClick={() => setItem({...item,specials:item.specials.filter((_,i)=>i!==index)})}><Trash2 size={16}/></button></div>)}</div>
      </div></form>
