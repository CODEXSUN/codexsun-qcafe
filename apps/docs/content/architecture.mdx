...
The framework kernel is tenancy-neutral. A tenancy add-on is optional and must not become an implicit platform dependency.
...
Composition roots only register modules and adapters. A module keeps its behavior, data, routes, events, and migrations in its own folder.
