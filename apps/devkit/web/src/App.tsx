...
const devKitTopology: InterfaceTopologySection[] = [
  ...
  { id: "05", name: "Developer tool list", scope: "Project drawer", description: "Developer tools that do not belong to the control plane." },
...
export function App() {
  ...
  return <div className="devkit-shell" {...topology.rootAttributes}>
    ...
    <main className="workspace ito-region" {...topology.regionProps("06")}><TopologyMarker id="06" topology={topology} /><header className="workspace-status ito-region" {...topology.regionProps("07")}><TopologyMarker id="07" topology={topology} /><span className="status-dot" /> Local developer session <span>Read-only workspace</span></header><section className="workspace-canvas ito-region" {...topology.regionProps("08")}><TopologyMarker id="08" topology={topology} /><div className="canvas-heading"><span>DEVELOPER WORKSPACE</span><h1>{activeTool}</h1><p>DevKit owns developer tools and keeps operational surfaces separate from the control plane.</p></div><div className="ito-region action-grid" {...topology.regionProps("09")}><TopologyMarker id="09" topology={topology} /><button><Terminal size={18} /> Open terminal</button><button><Code2 size={18} /> Inspect source</button><button><Activity size={18} /> Review agent run</button></div></section><footer className="workspace-footer ito-region" {...topology.regionProps("10")}><TopologyMarker id="10" topology={topology} />DevKit is ready for a public control-plane integration.</footer></main>
