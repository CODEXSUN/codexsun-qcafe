...
export function App() {
  ...
  const activePage = page.addonId === "zetro" ? "zetro-agent" : page.addonId === "ai-tasks" ? "ai-tasks" : page.addonId === "docs" ? "docs" : page.addonId === "device-chat" ? "device-chat" : page.addonId === "settings" ? "settings" : page.addonId === "chat" ? "chat" : "home";
  const pageSelector = <div className="grid gap-2 border-b border-border p-4 text-sm">
    ...
    <Select value={activePage} onValueChange={(value) => {
      setRequestedPage(value === "home" ? { ...overviewPage } : { view: "workspace", addonId: value === "chat" ? "chat" : value === "ai-tasks" ? "ai-tasks" : value === "settings" ? "settings" : value === "docs" ? "docs" : value === "device-chat" ? "device-chat" : "zetro", pageId: value === "zetro-agent" ? "agent" : value === "settings" ? "applications" : value === "docs" ? "architecture" : "" });
    }}>
  ...
  return <MainMdi applications={applications} identity={{ login: identity.profile?.login, workspaceLabel: identity.signedIn ? "Cloud workspace" : "Local workspace", onManageProfile: () => setRequestedPage({ view: "workspace", addonId: "settings", pageId: "identity" }), onSignOut: identity.signedIn ? identity.signOut : undefined }} addons={[overviewWorkspace, chatWorkspaceAddon, zetroWorkspaceAddon, aiTaskWorkspaceAddon, createDeviceChatWorkspaceAddon({ credentialStore: desktopCredentialStore("dcs-device") }), createDocsWorkspaceAddon(), createSettingsWorkspace(applications)]} onPageChange={onPageChange} requestedPage={requestedPage} topology={topology} />;
}
