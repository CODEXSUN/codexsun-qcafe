...
export function DeviceChatWorkspace({ credentialStore, sideCarTarget, topology }: { credentialStore?: CredentialStore; sideCarTarget?: HTMLElement | null; topology?: MdiTopologyAdapter }) {
  ...
  const sideCar = <MdiTopologyRegion id="dc2.1" topology={topology} className="space-y-2"><p className="px-2 text-xs font-semibold text-muted-foreground">DEVICES</p>{devices.map(device => <MdiTopologyRegion id="dc2.1.1" topology={topology} key={device.id}><button className={`flex w-full cursor-pointer items-center gap-3 rounded-lg px-3 py-3 text-left text-sm transition-colors hover:bg-accent hover:text-accent-foreground ${device.id === deviceId ? "bg-accent text-accent-foreground" : ""}`} onClick={() => setDeviceId(device.id)} type="button"><MonitorSmartphone className="size-4" /><span className="min-w-0"><span className="block truncate font-medium">{device.name}</span><span className="block text-xs text-muted-foreground">{device.kind}</span></span></button></MdiTopologyRegion>)}</MdiTopologyRegion>;
  ...
  return <MdiTopologyRegion id="dc1" topology={topology} className="flex h-full min-h-0 flex-col bg-background text-foreground">
    ...
    <MdiTopologyRegion id="dc1.1" topology={topology}><header className="flex items-center justify-between border-b border-border px-6 py-4"><div><h1 className="font-semibold">{selected?.name ?? "Device chat"}</h1><p className="text-sm text-muted-foreground">{status}</p></div></header></MdiTopologyRegion>
    <MdiTopologyRegion id="dc1.2" topology={topology} className="flex-1 overflow-y-auto px-6 py-6">{error && <p role="alert" className="mb-4 text-sm text-destructive">{error}</p>}{messages.filter(message => message.targetDeviceId === deviceId || message.senderDeviceId === activeDeviceId.current).map(message => <article className={`mb-3 max-w-xl rounded-xl border border-border p-3 text-sm ${message.senderDeviceId === activeDeviceId.current ? "ml-auto bg-accent" : "bg-card"}`} key={message.id}><p>{message.text}</p><p className="mt-2 text-xs text-muted-foreground">{new Date(message.sentAt).toLocaleString()}</p></article>)}</MdiTopologyRegion>
    <MdiTopologyRegion id="dc1.3" topology={topology}><form className="border-t border-border p-4" onSubmit={event => { event.preventDefault(); void send(); }}><div className="mx-auto flex max-w-3xl gap-3"><MdiTopologyRegion id="dc1.3.1" topology={topology} className="min-w-0 flex-1"><input aria-label="Device message" className="w-full rounded-lg border border-input bg-background px-3 py-2 outline-none focus:ring-2 focus:ring-ring" disabled={!deviceId} onChange={event => setDraft(event.target.value)} placeholder={deviceId ? "Send a device message" : "Select a device"} value={draft} /></MdiTopologyRegion><MdiTopologyRegion id="dc1.3.2" topology={topology}><Button aria-label="Send device message" className="cursor-pointer" disabled={!draft.trim() || !deviceId || status !== "Connected"} type="submit"><Send className="size-4" /></Button></MdiTopologyRegion></div></form></MdiTopologyRegion>
  </MdiTopologyRegion>;
}
