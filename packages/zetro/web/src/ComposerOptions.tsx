...
export function ComposerOptions({ activity, motion, onActivity, onMotion }: { activity: boolean; motion: boolean; onActivity: (value: boolean) => void; onMotion: (value: boolean) => void }) {
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    <PopoverTrigger asChild><Button type="button" aria-label="Agent flow options" title="Agent flow options" variant="outline" size="icon" className="size-8 rounded-xl bg-background text-muted-foreground shadow-none"><SlidersHorizontal className="size-4" /></Button></PopoverTrigger>
    <PopoverContent side="top" align="end" sideOffset={12} aria-label="Agent flow" className="w-64 rounded-2xl border-border p-3">
      <div className="flex items-center justify-between border-b border-border pb-2"><h2 className="text-sm font-semibold">Agent flow</h2><Button type="button" aria-label="Close agent flow" size="icon" variant="ghost" className="size-6" onClick={() => setOpen(false)}><X className="size-3" /></Button></div>
