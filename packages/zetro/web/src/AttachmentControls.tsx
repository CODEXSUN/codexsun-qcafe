...
export function AttachmentPreviews({ items, onChange, disabled }: { items: PromptAttachment[]; onChange: (items: PromptAttachment[]) => void; disabled: boolean }) {
  if (!items.length) return null;
  return <div className="flex max-h-32 flex-wrap gap-2 overflow-auto pb-2">
    {items.map((item, index) => <div key={`${item.name}-${index}`} className="flex max-w-full items-center gap-2 rounded-lg border border-border px-2 py-1 text-xs">
      ...
      <Button type="button" aria-label={`Remove ${item.name}`} disabled={disabled} variant="ghost" size="icon" className="size-6 cursor-pointer" onClick={() => onChange(items.filter((_, i) => i !== index))}><X className="size-3" /></Button>
    </div>)}
  </div>;
}
...
export function AttachmentControls({ items, onChange, disabled, onBusy, onError }: { items: PromptAttachment[]; onChange: (items: PromptAttachment[]) => void; disabled: boolean; onBusy: (busy: boolean) => void; onError: (error: string) => void }) {
  ...
  return <div className="flex flex-wrap items-center gap-1">
    ...
    <Button type="button" aria-label="Attach image" title="Attach image" variant="ghost" size="icon" disabled={disabled || loading || recording} className="cursor-pointer" onClick={() => image.current?.click()}><ImagePlus className="size-4" /></Button>
    <Button type="button" aria-label="Attach file" title="Attach file" variant="ghost" size="icon" disabled={disabled || loading || recording} className="cursor-pointer" onClick={() => file.current?.click()}><Paperclip className="size-4" /></Button>
    <Button type="button" aria-label={recording ? "Stop recording" : "Record voice"} title={recording ? "Stop recording" : "Record voice (up to 30 seconds)"} variant="ghost" size="icon" disabled={disabled || loading} className="cursor-pointer" onClick={() => void record()}>{recording ? <Square className="size-4 text-destructive" /> : <Mic className="size-4" />}</Button>
