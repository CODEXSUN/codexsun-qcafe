...
export function LeftIconDock({ addons, selectedAddonId, onAddonSelect, onViewChange, selectedView, topology }: LeftIconDockProps) {
  const settings = addons.find((addon) => addon.placement === "utility" && addon.id === "settings");
  const secondary = addons.filter((addon) => addon.placement === "secondary");
  return <aside aria-label="Primary navigation" className={topology ? "ito-region row-start-2 flex min-h-0 flex-col items-center border-r border-border bg-card py-3 [&>.technical-label]:!left-1/2 [&>.technical-label]:!top-1/2 [&>.technical-label]:-translate-x-1/2 [&>.technical-label]:-translate-y-1/2" : "row-start-2 flex min-h-0 flex-col items-center border-r border-border bg-card py-3"} {...topology?.regionProps("07")}>
    ...
    <div className="flex flex-col gap-2">
      {addons.filter((addon) => addon.placement !== "utility" && addon.placement !== "secondary").map((addon, index) => <MdiTopologyRegion key={addon.id} id={`7.${index + 1}`} topology={topology}><IconDockButton active={selectedAddonId === addon.id} icon={addon.icon} label={addon.label} onClick={() => onAddonSelect(addon)} /></MdiTopologyRegion>)}
    </div>
    <div className="mt-auto flex flex-col gap-2">
      {secondary.map((addon, index) => <MdiTopologyRegion id={`7.${index + 4}`} key={addon.id} topology={topology}><IconDockButton active={selectedAddonId === addon.id} icon={addon.icon} label={addon.label} onClick={() => onAddonSelect(addon)} /></MdiTopologyRegion>)}
      <MdiTopologyRegion id="7.5" topology={topology}><IconDockButton icon={Link} label="Link" /></MdiTopologyRegion>
      <MdiTopologyRegion id="7.6" topology={topology}><IconDockButton active={selectedAddonId === settings?.id} icon={Settings} label="Settings" onClick={settings ? () => onAddonSelect(settings) : undefined} /></MdiTopologyRegion>
    </div>
  </aside>;
}
