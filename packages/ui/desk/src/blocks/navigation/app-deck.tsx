...
export type DeskApplication = { id: string; name: string; onOpen?: () => void; webUrl?: string };
    ...
    <DropdownMenu.Portal>
      <DropdownMenu.Content onInteractOutside={(event) => { if ((event.detail.originalEvent.target as HTMLElement)?.closest?.('.technical-inspector')) event.preventDefault(); }} align="end" className="z-50 w-72 rounded-3xl border border-border bg-popover p-3 text-popover-foreground shadow-xl" sideOffset={8}>
        ...
        <div className="grid grid-cols-3 gap-2 rounded-2xl bg-muted/70 p-2">
          {applications.map(({ id, name, onOpen, webUrl }, index) => <MdiTopologyRegion key={id} id={`5.${index + 1}`} topology={topology}><DropdownMenu.Item asChild className="flex min-h-20 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl text-xs font-medium outline-none data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground" key={name}>{onOpen ? <button onClick={onOpen} type="button"><span className="grid size-9 place-items-center rounded-lg border border-border bg-card"><Boxes className="size-4" /></span>{name}</button> : <a href={webUrl} rel="noopener noreferrer" target="_blank"><span className="grid size-9 place-items-center rounded-lg border border-border bg-card"><Boxes className="size-4" /></span>{name}</a>}</DropdownMenu.Item></MdiTopologyRegion>)}
        </div>
