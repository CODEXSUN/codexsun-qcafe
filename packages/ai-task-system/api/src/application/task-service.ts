...
import type { TaskLifecycleObserver, TaskPlanner, TaskRepository, TaskWorker } from "./ports.js";

export class TaskService {
  private readonly active = new Set<string>();
  constructor(private readonly repository: TaskRepository, private readonly planner: TaskPlanner, private readonly worker: TaskWorker, private readonly observer?: TaskLifecycleObserver) {}
  list() { return this.repository.list(); }
  get(id: string) { return this.repository.get(id); }
  async create(input: CreateAiTask) {
    if (input.workCaseId) this.observer?.validate?.(input.workCaseId);
  ...
  start(id: string) {
    ...
    this.save(task.snapshot(), "task.started"); this.active.add(id);
  ...
  approve(id: string) {
    ...
    const task = TaskAggregate.restore(stored); task.complete(); this.save(task.snapshot(), "task.completed"); return task.snapshot();
  }
  ...
  private async execute(task: TaskAggregate) {
    ...
    for (const item of snapshot.workItems.filter((work) => work.status !== "completed")) {
      task.startWork(item.id); this.save(task.snapshot(), "work.started", { workItemId: item.id, agentId: item.agentId });
      try {
        ...
        task.completeWork(item.id, output); this.save(task.snapshot(), "work.completed", { workItemId: item.id, agentId: item.agentId });
      } catch (cause) {
        task.failWork(item.id, cause instanceof Error ? cause.message : "Agent execution failed.");
        this.save(task.snapshot(), "work.failed", { workItemId: item.id, agentId: item.agentId }); return;
      }
    }
  }
  private save(task: ReturnType<TaskAggregate["snapshot"]>, event: string, details: Record<string, unknown> = {}) {
    this.repository.save(task, event, details);
    this.observer?.record(task, event, details);
  }
}
